...
contract ERC777BaseToken is ERC777Token, ERC820Implementer {
    ...
    constructor(
        address _dataStorageAddress) internal {
    ...
    function authorizeOperator(address _operator) public {
        ...
        bool _status;
        if (DS.checkIsDefaultOperator(_operator)) {
            _status = false;
            DS.setRevokedOperator(_operator, msg.sender, _status);
        } else {
            _status = true;
            DS.setAuthorizedOperator(_operator, msg.sender, _status);
        }
    ...
    function isOperatorFor(address _operator, address _tokenHolder) public constant returns (bool) {
        return (_operator == _tokenHolder
            || DS.checkAuthorized(_operator, _tokenHolder)
            || (DS.checkIsDefaultOperator(_operator) && !DS.checkRevokedDefaultOperator(_operator, _tokenHolder)));
    }
    ...
    function requireMultiple(uint256 _amount) internal view {
        require(_amount.div(DS.granularity()).mul(DS.granularity()) == _amount);
    }
        ...
        internal
    {
        ...
        require(DS.balanceOf(_from) >= _amount); // ensure enough funds
        
        // sets the sender balance to balance - amount
        uint256 balanceSender = DS.balanceOf(_from);
        balanceSender = balanceSender.sub(_amount);
        DS.setBalance(_from, balanceSender);
        
        // sets the receiver balance to balance + amount
        uint256 balanceReceiver = DS.balanceOf(_to);
        balanceReceiver = balanceReceiver.add(_amount);
        DS.setBalance(_to, balanceReceiver);
        ...
        internal
    {
        ...
        uint256 burnedBalance = DS.balanceOf(_tokenHolder);
        burnedBalance = burnedBalance.sub(_amount);
        DS.setBalance(_tokenHolder, burnedBalance);
        
        // discards the burned amount from the totalSupply
        uint256 newTotalSupply = totalSupply();
        newTotalSupply = newTotalSupply.sub(_amount);
        DS.setTotalSupply(newTotalSupply);
